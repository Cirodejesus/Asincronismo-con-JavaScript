const fnAsync = () => { //Arrow function
    return new Promise((resolve, reject) => { //Retormna una promesa, con una promesa con una function anonima que nos expone resolve y reject con los cuales se maneja el flujo que suele tener una promesa.
...
const anotherFn = async () => { //la palabra async es para el cuerpo de la función, hace que la función devuelva una promesa.
    //la palabra await estará dentro de la lógica a implementar
    const something = await fnAsync(); //aquí nos está regresando una promesa, o que hace que el programa espere hasta que la variable(promesa) se resuelva para continuar..
